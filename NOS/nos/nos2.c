...
void papirPusac () {
	struct my_msgbuf buf;
    	...
    	for(;;) { 
        	...
        	key = 11111;
		char text[] = "Papir pusac:Uzeo bih sastojke sa stola\n";
		...
		sleep(1);
		printf("Papir pusac:Pocinjem motati cigaretu\n");
		sleep(3);
		printf("Papir pusac:Pocinjem pusiti cigaretu\n");
		sleep(4);
		printf("Papir pusac:Gotov s pusenjem\n");}
        	/*printf("Papir pusac: received: \"%s\". \nPapir pusac:Uzet cu!\n", buf.mtext);*/
        	}
    }
...
void duhanPusac () {
	struct my_msgbuf buf;
    	...
    	for(;;) { 
        	...
        	key = 11111;
		char text[] = "Duhan pusac:Uzeo bih sastojke sa stola\n";
		if ((msqid = msgget(key, 0600 | IPC_CREAT)) == -1) {
    			perror("msgget");
			exit(1);
    		}
		...
		sleep(1);
		printf("Duhan pusac:Pocinjem motati cigaretu\n");
		sleep(3);
		printf("Duhan pusac:Pocinjem pusiti cigaretu\n");
		sleep(4);
		printf("Duhan pusac:Gotov s pusenjem\n");
		}
        	/*printf("Duhan pusac: received: \"%s\". \nDuhan pusac:Uzet cu!\n", buf.mtext);*/
        	}
	}
	...
	struct my_msgbuf buf;
    	...
    	for(;;) { 
        	...
        	if (buf.mtype == 3) {
        		...
        		key = 11111;
        		char text[] = "Sibice pusac:Uzeo bih sastojke sa stola\n";
        		if ((msqid = msgget(key, 0600 | IPC_CREAT)) == -1) {
	    			perror("msgget");
				exit(1);
	    		}
			...
			sleep(1);
			printf("Sibice pusac:Pocinjem motati cigaretu\n");
			sleep(3);
			printf("Sibice pusac:Pocinjem pusiti cigaretu\n");
			sleep(4);
			printf("Sibice pusac:Gotov s pusenjem\n");}
        		/*printf("Sibice pusac: received: \"%s\". \nSibice pusac:Uzet cu!\n", buf.mtext);*/
        	}
	}
}
...
void trgovac () {
	struct my_msgbuf buf;
    	...
    	while(1){
	    	...
	    	if (msgrcv(msqid, (struct msgbuf *)&buf, sizeof(buf)-sizeof(long), num, 0) == -1) {
            		perror("msgrcv");
            		exit(1);
            		}
		printf("Trgovac: received: \"%s\". \nTrgovac:Uzivaj!\n", buf.mtext);
		printf("Gotov prvi krug");
		}
